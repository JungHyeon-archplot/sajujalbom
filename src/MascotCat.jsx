...
export default function MascotCat({ className = '', variant = 'full' }) {
  ...
  const haloId = `mascotHalo-${uid}`
  const furId = `mascotFur-${uid}`
  const mountainId = `mascotMountain-${uid}`
  const inkId = `mascotInk-${uid}`
  const buddy = variant === 'buddy'

  return (
    <svg
      viewBox={buddy ? '48 38 128 172' : '0 0 200 220'}
      className={`mascot ${className}`.trim()}
      role="img"
      aria-hidden={buddy ? 'true' : undefined}
      aria-label={buddy ? undefined : '사주잘봄 마스코트 고양이'}
    >
      <defs>
      ...
      {/* 해 후광 */}
      <circle className="mascot-halo" cx="100" cy="84" r="66" fill={`url(#${haloId})`} />
      <circle
        cx="100"
        cy="84"
        r="46"
      ...
      {!buddy && (
        <>
          <g opacity="0.75">
            <path d="M6 168 L40 118 L74 168 Z" fill={`url(#${mountainId})`} />
            <path d="M126 168 L160 112 L196 168 Z" fill={`url(#${mountainId})`} />
            <path
      ...
      <g stroke={`url(#${inkId})`} strokeWidth="2.2" strokeLinejoin="round" strokeLinecap="round">
        ...
        <path d="M70 78 L64 46 L92 62 Z" fill={`url(#${furId})`} />
        <path d="M130 78 L136 46 L108 62 Z" fill={`url(#${furId})`} />
        <path
        ...
        <ellipse cx="100" cy="90" rx="35" ry="32" fill={`url(#${furId})`} />
        <path
