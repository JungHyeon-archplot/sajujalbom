...
// 프롬프트는 서버에서만 만듭니다(클라이언트 문장을 그대로 넘기면 무료 LLM 창구가 됩니다).
...
export default async function handler(req) {
  ...
  const blocked = checkUsageLimit(kind, req, user?.id)
    ...
    async start(controller) {
      try {
        ...
        if (!result.ok) {
          ...
          await archiveReading(prompt.user, result.text, user?.id || null)
        }
        controller.close()
      } catch (err) {
        try {
...
function checkUsageLimit(kind, req, userId) {
